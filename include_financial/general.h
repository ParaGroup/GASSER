...
#define NUM_IT 10		//how many time perform the computation (used for testing)

//#define PREGENERATE	//the synthetic generator pregenerates all the tuples (faster rates but higher memory occupancy)
...
*/
 struct winresult_t{
    ...
    //parabola parameter
	float p0_bid,p1_bid,p2_bid;
	float p0_ask,p1_ask,p2_ask;
    //candlestick parameters
	float open_bid,close_bid,high_bid,low_bid;
	float open_ask,close_ask,high_ask,low_ask;
	...
	long computation_time=0; //computation time used for testing
 ...
 /**
   Definition of a result for the toy examples. This simply maitains the sum for all the bid and asks in window
 */

 struct toy_winresult_t{
	 int64_t id; //id of the task that has triggered the computation
	 long long timestamp; //timestamp for monitoring purposes
	 int type; //class
	 double sum_bids;
